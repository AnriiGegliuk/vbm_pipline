...
# def segment_brain_images(input_file: str, tpm_path: str, output_dir: str, iteration: int) -> str:
...
#     output_subdir = os.path.join(output_dir, f'{iteration}_segment')
...
# def mask_brain_images(segmentation_dir: str, output_dir: str, subject_id: str, iteration: int) -> str:
...
# working but no idea data strcture
# from nipype.interfaces.spm import NewSegment
# from nipype import Node, Workflow
# from nipype.interfaces.io import DataSink
# import os
...
#     output_subdir = os.path.join(subject_output_dir, f'{iteration}_segment')
#     os.makedirs(output_subdir, exist_ok=True)
...
#     # Save the mask image data
#     mask_output_dir = os.path.join(output_dir, subject_id, f'{iteration}_mask')
#     os.makedirs(mask_output_dir, exist_ok=True)
#     mask_file_path = os.path.join(mask_output_dir, f'mask_{subject_id}.nii')
#     mask_nifti = nb.Nifti1Image(mask_image_data, affine=c1_nifti.affine)
#     nb.save(mask_nifti, mask_file_path)
...
# def segment_brain_images(input_file: str, tpm_path: str, output_dir: str, iteration: int, subject_id: str) -> str:
#     """ Segment brain MRI images using Nipype-SPM12. """
...
def segment_brain_images(input_file: str, tpm_path: str, iteration: int, subject_id: str) -> str:
    """ Segment brain MRI images using Nipype-SPM12. """
    ...
    subject_output_dir = os.path.dirname(input_file)
    output_subdir = os.path.join(subject_output_dir, f'{iteration}_segment')
    os.makedirs(output_subdir, exist_ok=True)
...
def mask_brain_images(segmentation_dir: str, subject_folder: str, subject_id: str, iteration: int) -> str:
    ...
    # Save the mask image data
    mask_output_dir = os.path.join(subject_folder, f'{iteration}_mask')
    os.makedirs(mask_output_dir, exist_ok=True)
    mask_file_path = os.path.join(mask_output_dir, f'mask_{subject_id}.nii')
    mask_nifti = nb.Nifti1Image(mask_image_data, affine=c1_nifti.affine)
    nb.save(mask_nifti, mask_file_path)
