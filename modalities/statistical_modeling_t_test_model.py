...
def find_conditions_and_subjects(root_dir: str) -> (List[str], List[str], List[str]):
    """ Automatically detect conditions and subjects, ignoring specified folders. """
    ignore_folders = {'Dartel', 'statistical_analysis'}
    conditions = sorted([d for d in os.listdir(root_dir) if os.path.isdir(os.path.join(root_dir, d)) and d not in ignore_folders], key=str.lower)
    
    condition1_subjects = []
    condition2_subjects = []
    
    if len(conditions) < 2:
        print("Not enough conditions found for analysis.")
        return [], [], []
...
def create_two_sample_t_test_model(root_dir: str, contrasts_name: str) -> List[str]:
    ...
    # Print detected conditions
    print(f"Detected conditions: {conditions}")
    if not condition1_subjects or not condition2_subjects:
        print(f"Detected subjects for {conditions[0]}: {condition1_subjects}")
        print(f"Detected subjects for {conditions[1]}: {condition2_subjects}")
        print("Missing files for one or both groups.")
        return []
