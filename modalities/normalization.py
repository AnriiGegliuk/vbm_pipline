...
#     return filename.split('T2_')[-1].split('.')[0] #:TODO this might be not ideal s 
...
# testing according to new data structure working as expected
...
def get_subject_id_from_filename(filename: str) -> str:
    return filename.split('T2_')[-1].split('.')[0]  # Extract subject ID from filename
...
def normalize_brain_images(root_derivatives_dir_path: str) -> List[str]:
    ...
    dartel_dir = os.path.join(root_derivatives_dir_path, 'Dartel')

    # Collecting c1 files
    for file in os.listdir(os.path.join(dartel_dir, 'c1')):
        if file.endswith('.nii'):
            c1_file = os.path.join(dartel_dir, 'c1', file)
            c1_files.append(c1_file)

    # Collecting flow field files
    for file in os.listdir(os.path.join(dartel_dir, 'output', 'flowfields')):
        if file.startswith('u_') and file.endswith('_Template.nii'):
            flow_field_file = os.path.join(dartel_dir, 'output', 'flowfields', file)
            flow_field_files.append(flow_field_file)
    ...
    # Set a DataSink node.
    sink_node = Node(DataSink(), name='data_sink')
    sink_node.inputs.base_directory = os.path.join(dartel_dir, 'output', 'warped_files')
    ...
    # Collect the paths of the output files saved in the analysis.
    output_files = []
    output_subdir_path = os.path.join(dartel_dir, 'output', 'warped_files')
    for root, dirs, files in os.walk(output_subdir_path):
        for file in files:
            output_files.append(os.path.join(root, file))
